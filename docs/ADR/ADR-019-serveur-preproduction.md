...
- **À prévoir** :
  ...
  - **docker-compose de préprod** : **délégué à Claude Code** (hors périmètre ADR — l'ADR fixe la décision, pas l'implémentation). ✅ **Livré dans [`deploy/preprod/`](../../deploy/preprod/)** (1ᵉʳ juin 2026) : Dockerfiles API/web, `docker-compose.yml` (postgres + api + web + caddy + backup), `Caddyfile` (domaine unique, `/api` routé, TLS Let's Encrypt), secrets en fichiers montés, `pg_dump` quotidien, migrations au démarrage. Cf. `deploy/preprod/README.md`.
